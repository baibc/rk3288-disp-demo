		       ...
		       unsigned long arg)
{
	...
	case RK_FBIOSET_OVERLAY_STA:
		if (copy_from_user(&ovl, argp, sizeof(ovl)))
			return -EFAULT;
		dev_drv->ops->ovl_mgr(dev_drv, ovl, 1);  //by blues 
		break;
...
//	win->area[0].cbr_start = fix->smem_start+stride*yvir;//fix->mmio_start;
 ...
 /*       if (win->format == ARGB888||win->format == ABGR888) {
        ...
        dev_drv->ops->ovl_mgr(dev_drv, 0, 1);
        dev_drv->ops->cfg_done(dev_drv);*///by blues
...
static int rk_fb_alloc_buffer_fb2(struct fb_info *fbi, int fb_id)   //add by blues 
{
	struct rk_fb *rk_fb = platform_get_drvdata(fb_pdev);
	struct rk_lcdc_driver *dev_drv = (struct rk_lcdc_driver *)fbi->par;
	struct rk_lcdc_win *win = NULL;
...
#endif
	/*####by blues add 20160923*///初始化fb2,给fb2分配buffer
		struct fb_info *main_fbi_2 = rk_fb->fb[2];
		main_fbi_2->fbops->fb_open(main_fbi_2, 2);
		rk_fb_alloc_buffer_fb2(main_fbi_2, 2);	/* only alloc memory for main fb2 */
		main_fbi_2->fbops->fb_set_par(main_fbi_2);
		main_fbi_2->fbops->fb_pan_display(&main_fbi_2->var, main_fbi_2);

	/*####by blues add 20160923*/
	return 0;
